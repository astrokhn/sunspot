...
def save_to_notion(user_name, sunspot_count, memo):
    ...
    notion.pages.create(
        ...
        properties={
            "사용자": {"title": [{"text": {"content": user_name}}]},
...
st.title("🌞 AI가 본 태양")
st.write("휴대폰으로 촬영한 태양 사진을 업로드하면, AI가 흑점을 찾아주고 개수를 알려줍니다.")

uploaded_file = st.file_uploader("태양 이미지를 업로드하세요 (JPG/PNG)", type=["jpg", "jpeg", "png"])
...
if uploaded_file is not None:
    ...
    with open("sunspot_result.jpg", "rb") as f:
        st.download_button("📥 결과 이미지 다운로드", f, file_name="sunspot_result.jpg", mime="image/jpeg")
